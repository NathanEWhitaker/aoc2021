...
impl BingoBoard {
    fn new(rows: Vec<Vec<i32>>) -> Self {
        let cells: [BingoCell; 25] = rows.into_iter()
            ...
            .try_into()
            .unwrap();
        BingoBoard{cells, score: None}
    }

    fn row_n_filled(&self, row: usize) -> bool {
        let row_start = row * 5;
        self.cells[row_start..row_start+5].iter().all(|c| c.marked)
    }
    ...
    fn winning(&mut self, winning_ball: i32) -> &Option<i32> {
...
fn board_lines_to_board(lines: &[String]) -> BingoBoard {
...
fn main() {
    ...
    // run first five iterations since no solves are possible
    for i in 0..5 {
        let ball = balls[i];
    ...
    let mut last_winning_score: Option<i32> = None;
    for i in 5..(balls.len()) {
        let ball = balls[i];
        val_to_cells.get(&ball).unwrap().iter().for_each(|(b, c)| {
            let board = &mut boards[*b];
            if board.score.is_none() {
                board.mark_cell(*c);
